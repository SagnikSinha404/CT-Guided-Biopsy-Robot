...
def draw_hand_landmarks(frame, hand_landmarks):
    ...
    for idx in PALM_LANDMARK_IDS:
        cv2.circle(frame, points[idx], 7, (0, 0, 255), -1)
...
detectorH = vision.HandLandmarker.create_from_options(options)
...
while True:
    ...
    result = detectorH.detect_for_video(
        mp_image,
        timestamp_ms
    )
    ...
    if result.hand_landmarks and ids is None:
        ...
        if success:
            ...
            cv2.putText(
                frame,
                f"Palm x:{x:.1f} y:{y:.1f} z:{z:.1f} mm",
                (10, 30),
                cv2.FONT_HERSHEY_PLAIN,
                1.5,
                (0, 0, 255),
            ...
            cv2.putText(
                frame,
                f"Smooth x:{smooth_x:.1f} Smooth y:{smooth_y:.1f}",
                (10, 60),
                cv2.FONT_HERSHEY_PLAIN,
                1.5,
                (255, 0, 0),
